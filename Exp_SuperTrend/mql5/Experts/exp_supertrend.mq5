...
int OnInit()
  {
//---- получение хендла индикатора SuperTrend
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"SuperTrend",CCIPeriod,ATRPeriod,Level,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора SuperTrend");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   min_rates_total=int(MathMax(CCIPeriod,ATRPeriod)+SignalBar+1);
